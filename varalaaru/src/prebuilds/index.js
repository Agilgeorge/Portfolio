import saalai from "../assets/saalai.png";
import rider from "../assets/rider.png";
import ikigai from "../assets/ikigai.png";
import Altair from "../assets/Altair.png";
import otaku from "../assets/otaku.png";

export const PROJECTS = [
  {
    title: "SAALAIGAL",
    image: saalai,
    description:
      "Saalaigal is an app designed to improve road transport infrastructure by allowing users to report issues such as potholes, streetlight malfunctions, and other road-related complaints.",
    technologies: ["Flutter", "Dart", "Firebase", "git"],
  },
  {
    title: "RIDER-01",
    image: rider,
    description:
      "Rider-01 is a platform for moto riders to connect and join ride events around them, fostering a community of biking enthusiasts.",
    technologies: ["React JS", "TailWind CSS", "NodeJS", "Vite"],
  },
  {
    title: "Otaku-Hub",
    image: otaku,
    description:
      "A platform designed for anime enthusiasts to connect, shop for anime merchandise, stay updated with the latest anime news, and share their thoughts through blogs.",
    technologies: ["OpenWeaver Studio", "Photoshop", "Figma"],
  },
  {
    title: "Ikigai",
    image: ikigai,
    description:
      "A platform for creating and publishing blog posts, with features like rich text editing, commenting, and user profiles.",
    technologies: ["Framer", "CSS", "MongoDB", "React"],
  },
  {
    title: "Altair-Disease prediction",
    image: Altair,
    description:
      "A data-driven project designed to predict multiple diseases using blood sample dataset.",
    technologies: ["RapidMiner", "Kaggle", "Pitch"],
  },
];

export const CONTACT = {
  address: "#476,Valluvar Nagar,Ennore,chennai-57 ",
  email: "mailtoagilgeorge24@gmail.com",
};